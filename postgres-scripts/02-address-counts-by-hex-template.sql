...
INSERT INTO hex.{0}
  ...
  SELECT CASE WHEN left(hex_pid, 1) = '-' THEN split_part(hex_pid, '-', 2)::integer * -1 ELSE split_part(hex_pid, '-', 1)::integer END AS x,
       ...
       count_2010,
       count_current,
       count_current - count_2010 AS difference,
       CASE WHEN count_2010 > 0 THEN (count_current::float / count_2010::float * 100.0)::integer - 100 ELSE count_current * 100.0 END AS percent,
       null::geometry(polygon,4326) AS geom
  FROM sqt
  WHERE count_current - count_2010 > {3};
